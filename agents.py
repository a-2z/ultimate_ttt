...
class TimedMinmaxAgent(Agent):
    ...
    def make_move(self, board):
        ...
        while (time.time() - s_time) < self.max_time:
            ...
            alpha = float("-inf")
            beta = float("inf")
            score, best_move = self.maximize(
                board.copy(), alpha, beta, depth, None)
    ...
    def maximize(self, board, alpha, beta, depth, move):
        ...
        for i in range(valid_moves.shape[0]):
            ...
            move_val, min_move = self.minimize(
                board, a, b, depth - 1, made_move)
    ...
    def minimize(self, board, alpha, beta, depth, move):
        ...
        for i in range(valid_moves.shape[0]):
            made_move = (valid_moves[i, 0], valid_moves[i, 1],
                         valid_moves[i, 2], valid_moves[i, 3])
            board.move(made_move, -self.player)
            move_val, max_move = self.maximize(
                board, a, b, depth - 1, made_move)
        ...
        if self.root is None:
            ...
            for child in self.root.children:
                if np.all(child.board.get_state() == board.get_state()):
                    ...
                    break
            if not found:
                self.root = Node(-self.player, board.copy(),
                                 None, self.c, None)
        ...
        if self.board.result != 0:
            ...
            for i in range(valid_moves.shape[0]):
                ...
                child_node = Node(child_player, child_board,
                                  valid_moves[i, :], self.c, parent=self)
